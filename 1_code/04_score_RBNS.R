#################################################################################################################################
### But: Obtenir la distribution prédictive des RBNS avec le modèle d'Antonio et Plat                                         ###
...
# Fonction qui simule le montant de RBNS. Retourne un data frame dont chaque ligne est une simulation ---------------------------
simulate_RBNS <- function(nsim) {
  f <- function(seed) {
    set.seed(seed)

    events <- train_RBNS %>% 
      mutate(events = map(dev_time_at_valuation, simulate_claim_path)) %>% 
...
RBNS_sim <- simulate_RBNS(5000)
saveRDS(RBNS_sim, file = here("2_pipeline", "04_score_RBNS", "RBNS_sim.RDS"))
