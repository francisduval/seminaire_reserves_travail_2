#################################################################################################################################
### But: Visualiser les distributions prédictives du modèle individuel, pour les RBNS et IBNR séparément                      ###
...
p4 <- plot_pred_dist(nb_events_T1_RBNS_pd, obs = nb_events_T1_RBNS_obs, sub = "Événements RBNS (type 1)", xlab = "Nombre", bw = 20)
p5 <- plot_pred_dist(nb_events_T2_RBNS_pd, obs = nb_events_T2_RBNS_obs, sub = "Événements RBNS (type 2)", xlab = "Nombre", bw = 10)
p6 <- plot_pred_dist(nb_events_T3_RBNS_pd, obs = nb_events_T3_RBNS_obs, sub = "Événements RBNS (type 3)", xlab = "Nombre", bw = 10)

pdf(file = here("3_output", "results_RBNS.pdf"), width = 9, height = 5)
...
p12 <- plot_pred_dist(nb_events_T2_T3_IBNR_pd, obs = nb_events_T2_T3_IBNR_obs, sub = "Événements IBNR (type 2 et 3)", xlab = "Nombre", bw = 6)

pdf(file = here("3_output", "results_IBNR.pdf"), width = 9, height = 5)
